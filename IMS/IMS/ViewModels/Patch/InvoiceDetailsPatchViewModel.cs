...
{
    internal class InvoiceDetailsPatchViewModel : ViewModelBase
    {
        ...
        public InvoiceDetailsPatchViewModel()
        {
        ...
        private void cmdGenerateExcel_Execute(object obj)
        {
            try
            {
                ...
                {
                    ObservableCollection<InvoiceCustomerDetails> lstInvoiceDetails = pxy.InvoiceDetailsPatch(fromInvoiceNo, toInvoiceNo, fromInvoiceDate, toInvoiceDate, Global.UserID);
                    ...
                    using (ReportViewer reportViewer = new ReportViewer())
                    {
                        ...
                        reportViewer.LocalReport.DataSources.Add(new ReportDataSource("InvoiceDetailsDataSet", lstInvoiceDetails));
                        reportViewer.LocalReport.ReportPath = System.Windows.Forms.Application.StartupPath + "//Views//Reports//InvoiceDetailsPatch.rdlc";
                        reportViewer.RefreshReport();
                        byte[] Bytes = reportViewer.LocalReport.Render(format: "Excel", deviceInfo: "");
                        using (FileStream stream = new FileStream(folderPath + "//InvoiceDetailsPatch.xls", FileMode.Create))
                        {
