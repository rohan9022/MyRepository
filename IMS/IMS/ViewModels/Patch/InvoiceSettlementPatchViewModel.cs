...
{
    internal class InvoiceSettlementPatchViewModel : ViewModelBase
    {
        ...
        private DateTime _fromSettlementDate;

        public DateTime fromSettlementDate
        {
            get { return _fromSettlementDate; }
            set
            {
                _fromSettlementDate = value;
                OnPropertyChanged(this, "fromSettlementDate");
            }
        }

        private DateTime _toSettlementDate;

        public DateTime toSettlementDate
        {
            get { return _toSettlementDate; }
            set
            {
                _toSettlementDate = value;
                OnPropertyChanged(this, "toSettlementDate");
            }
        }
        ...
        public DelegateCommand<object> cmdBlankInvoiceDate { get; set; }
        public DelegateCommand<object> cmdBlankSettlementDate { get; set; }
        ...
        public InvoiceSettlementPatchViewModel()
        {
            ...
            cmdBlankInvoiceDate = new DelegateCommand<object>(cmdBlankInvoiceDate_Execute);
            cmdBlankSettlementDate = new DelegateCommand<object>(cmdBlankSettlementDate_Execute);
        ...
        private void ClearValues()
        {
            ...
            fromSettlementDate = toSettlementDate = new DateTime(1900, 01, 01);
            ProductID = string.Empty;
            Busy = btnErrorFile = false;
            btnUpload = true;
        }

        private void cmdBlankInvoiceDate_Execute(object obj)
        {
            fromInvoiceDate = toInvoiceDate = new DateTime(1900, 01, 01);
        }

        private void cmdBlankSettlementDate_Execute(object obj)
        {
            fromSettlementDate = toSettlementDate = new DateTime(1900, 01, 01);
        }
        ...
        private void cmdUploadExcel_Execute(object obj)
        {
            try
            {
                ...
                IMS.InvoiceServiceRef.TupleOfstringstringboolean objTuple = pxyInvoice.UploadPatchSheet(IMSLibrary.Class.Helper.ConvertToByte(fileName.ToString()), safeFileName, Global.UserID, Const.InvoiceSettlementSheetType);
        ...
        private void cmdGenerateExcel_Execute(object obj)
        {
            try
            {
                ...
                ////    return;
                ////}
                if (fromSettlementDate > toSettlementDate)
                {
                    UIHelper.ShowErrorMessage("From Settlement Date cannot not be greater than To Date");
                    return;
                }
                ////if ((toSettlementDate - fromSettlementDate).Days > 31)
                ////{
                ////    UIHelper.ShowErrorMessage("Days Difference should not be greater than 31");
                ////    return;
                ////}
                ...
                {
                    ObservableCollection<SettlementDetails> lstInvoiceSettlement = pxy.InvoiceSettlementPatch(fromInvoiceNo, toInvoiceNo, fromInvoiceDate, toInvoiceDate, ProductID, fromSettlementDate, toSettlementDate, Global.UserID);
                    if (lstInvoiceSettlement == null || lstInvoiceSettlement.Count == 0)
                    {
                    ...
                    using (ReportViewer reportViewer = new ReportViewer())
                    {
                        ...
                        reportViewer.LocalReport.DataSources.Add(new ReportDataSource("InvoiceSettlementDataSet", lstInvoiceSettlement));
                        reportViewer.LocalReport.ReportPath = System.Windows.Forms.Application.StartupPath + "//Views//Reports//InvoiceSettlementPatch.rdlc";
                        reportViewer.RefreshReport();
                        byte[] Bytes = reportViewer.LocalReport.Render(format: "Excel", deviceInfo: "");
                        using (FileStream stream = new FileStream(folderPath + "//InvoiceSettlementPatch.xls", FileMode.Create))
                        {
